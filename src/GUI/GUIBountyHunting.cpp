...
void bountyHuntingDialogueOptionClicked()
{
	...
	else
	{
		...
		if (pParty->monster_id_for_hunting[house])
		{
			int bounty = 100 * pMonsterStats->GetMonsterInfo(pParty->monster_id_for_hunting[house]).uLevel;
...
std::string bountyHuntingText()
{
	...
	const auto& monster_info = pMonsterStats->GetMonsterInfo(bountyHunting_monsterId);

	// TODO(captainurist): what do we do with exceptions inside fmt?
	return fmt::sprintf(bountyHunting_text,
		fmt::format("\f{:05}{}\f{:05}", colorTable.PaleCanary.c16(), monster_info.pName, colorTable.White.c16()).c_str(), 100 * monster_info.uLevel);
}
