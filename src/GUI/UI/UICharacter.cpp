...
void GUIWindow_CharacterRecord::CharacterUI_AwardsTab_Draw(Player* player)
{
	...
	{
		std::string str;
		const char* v6 = pAwards[achieved_awards[i]].pText.c_str();
...
static void CharacterUI_DrawItem(int x, int y, ItemGen* item, int id, Texture* item_texture, bool doZDraw)
{
	...
	else if (!item->IsIdentified() && (engine->config->gameplay.ShowUnidentifiedItem.value() || id))
	{
...
void GUIWindow_CharacterRecord::CharacterUI_StatsTab_Draw(Player* player)
{
	...
	if (player->uQuickSpell != SPELL_NONE)
		spellName = pSpellStats->pInfos[player->uQuickSpell].pShortName.c_str();
...
void FillAwardsData()
{
	...
	for (int i = 1; i < 105; ++i)
	{
		if (_449B57_test_bit(pPlayer->_achieved_awards_bits, i) && !pAwards[i].pText.empty())
...
// int v8; // eax@29
	...
	{  // hold item
		...
		if (!pPlayers[pParty->getActiveCharacter()]->CanEquip_RaceAndAlignmentCheck(pickeditem))
		{  // special item checks
			pPlayers[pParty->getActiveCharacter()]->playReaction(SPEECH_CantEquip);
			return;
		}

		if (pParty->pPickedItem.uItemID == ITEM_QUEST_WETSUIT)
		{  // wetsuit check is done above
		...
		case EQUIP_AMULET:
			...
			{  // hasnt got the skill to use that
				pPlayers[pParty->getActiveCharacter()]->playReaction(SPEECH_CantEquip);
				return;
			}

			if (pPlayers[pParty->getActiveCharacter()]->hasUnderwaterSuitEquipped() &&
				(pEquipType != EQUIP_ARMOUR || engine->IsUnderwater()))
...
// equippos = 0;
			...
			{  // rings displayed
				...
				{  // not click on right area so exit
					return;
				}
		...
		case EQUIP_SHIELD:  //Щит
			...
			else
			{
				...
				if (!twohandedequip)
				{  // обычная установка щита на пустую
				  // руку
		...
		case EQUIP_TWO_HANDED:
			...
			else
			{
				...
				{
					if (shieldequip)
					{  // взять двуручный меч когда есть
					  // щит(замещение щитом)
						shieldequip--;
						memcpy(&_this, &pParty->pPickedItem, sizeof(_this));
						pPlayers[pParty->getActiveCharacter()]
