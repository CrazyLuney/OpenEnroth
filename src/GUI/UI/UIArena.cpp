...
void ArenaFight()
{
	...
	*(&pCurrentFrameMessageQueue->uNumMessages + 3 *
	...
	{
		const auto& monster_info = pMonsterStats->GetMonsterInfo(i);

		if (monster_info.uAIType != MONSTER_AI_WIMP)
		{
			if (!MonsterStats::BelongsToSupertype(monster_info.uID, MONSTER_SUPERTYPE_8))
			{
				if (monster_info.uLevel >= v4)
				{
					if (monster_info.uLevel <= num_monsters)
						v23[v6++] = i;
				}
			}
		}
