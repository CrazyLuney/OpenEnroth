...
{
	CharacterUI_DrawTooltip(localization->GetAttributeName(attribute), localization->GetAttributeDescription(attribute));
}
...
void CharacterUI_SkillsTab_ShowHint()
{
	...
	else
	{
		CharacterUI_DrawTooltip(
			...
			localization->GetAttributeDescription(TEMP_ATTRIBUTE_SKILL_POINTS));
	}
}
...
void CharacterUI_StatsTab_ShowHint()
{
	...
	for (pStringNum = 0; pStringNum < stat_string_coord.size(); ++pStringNum)
	{
		const bool outside =
			...
			(pt.x > stat_string_coord[pStringNum].x + stat_string_coord[pStringNum].width) ||
			(pt.y < stat_string_coord[pStringNum].y) ||
			(pt.y > stat_string_coord[pStringNum].y + stat_string_coord[pStringNum].height);
	...
	{
	case 0:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_MIGHT);
		break;
	case 1:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_INTELLECT);
		break;
	case 2:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_PERSONALITY);
		break;
	case 3:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_ENDURANCE);
		break;
	case 4:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_ACCURACY);
		break;
	case 5:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_SPEED);
		break;
	case 6:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_LUCK);
		break;
	case 7:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_HIT_POINTS);
		break;
	case 8:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_SPELL_POINTS);
		break;
	case 9:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_ARMOR_CLASS);
		break;
	case 10:  // Player Condition
	{
		auto str = std::string(localization->GetAttributeDescription(TEMP_ATTRIBUTE_CONDITION)) + "\n";
		...
		if (!str.empty())
			CharacterUI_DrawTooltip(
				localization->GetAttributeName(TEMP_ATTRIBUTE_CONDITION),
				str);
		break;
	}

	case 11:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_QUICK_SPELL);
		break;

	case 12:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_AGE);
		break;

	case 13:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_LEVEL);
		break;
	...
	case 15:  // Attack Bonus
	{
		...
		description = fmt::format("{}\n\n{}", localization->GetAttributeDescription(TEMP_ATTRIBUTE_ATTACK_BONUS), description.c_str());
		CharacterUI_DrawTooltip(
			localization->GetAttributeName(TEMP_ATTRIBUTE_ATTACK_BONUS),
			description);
	}
	break;

	case 16:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_ATTACK_DAMAGE);
		break;
	...
	case 17:  // Missle Bonus
	{
		...
		description = fmt::format("{}\n\n{}", localization->GetAttributeDescription(TEMP_ATTRIBUTE_SHOOT_BONUS), description);
		CharacterUI_DrawTooltip(
			localization->GetAttributeName(TEMP_ATTRIBUTE_SHOOT_BONUS),
			description);
	}
	break;

	case 18:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_SHOOT_DAMAGE);
		break;

	case 19:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_FIRE);
		break;

	case 20:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_AIR);
		break;

	case 21:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_WATER);
		break;

	case 22:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_EARTH);
		break;

	case 23:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_MIND);
		break;

	case 24:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_BODY);
		break;

	case 25:
		CharacterUI_DrawAttributeTooltip(TEMP_ATTRIBUTE_SKILL_POINTS);
		break;

	case 26:
		CharacterUI_DrawTooltip(
			localization->GetClassName(pPlayers[pParty->getActiveCharacter()]->classType),
			localization->GetClassDescription(pPlayers[pParty->getActiveCharacter()]->classType));
		break;
...
void UI_OnMouseRightClick(int mouse_x, int mouse_y)
{
		...
		// главном меню показывает всплывающее окно)
	{
		...
		else if ((int)pX > pViewport->uViewportBR_X)
		{
			if (pY >= 130)
			{
				if (pX >= 476 && pX <= 636 && pY >= 240 && pY <= 300)
				{  // buff_tooltip zone
					popup_window.sHint.clear();
					popup_window.uFrameWidth = 400;
					popup_window.uFrameHeight = 200;
		...
		for (GUIButton* pButton : pGUIWindow_CurrentMenu->vButtons)
		{
				...
				(signed int)pY < (signed int)pButton->uW)
			{
				...
				case UIMSG_0:  // stats info
					...
					pStr = localization->GetAttributeName((int)pButton->msg_param % 7);
					break;
				...
				case UIMSG_PlayerCreationClickOK:  // OK Info
					popup_window.sHint = localization->GetString(LSTR_ACCEPT_PARTY_HINT);
					pStr = localization->GetString(LSTR_OK_BUTTON);
					break;
				case UIMSG_PlayerCreationClickReset:  // Clear info
					popup_window.sHint = localization->GetString(LSTR_RESET_HINT);
				...
				if (pButton->msg > UIMSG_44 && pButton->msg <= UIMSG_PlayerCreationRemoveDownSkill)
				{
					// Sellected skills info
					pY = 0;
					if (pParty->pPlayers[pButton->msg_param].GetSkillIdxByOrder(pButton->msg - UIMSG_48) != PLAYER_SKILL_INVALID)
					{
						static std::string hint_reference;
						hint_reference = CharacterUI_GetSkillDescText(pButton->msg_param, pParty->pPlayers[pButton->msg_param].GetSkillIdxByOrder(pButton->msg - UIMSG_48));
...
{  // needs cleaning
	...
	if (pX <= 13 || pX >= 462)
	{
		// items out of inventory(вещи вне инвентаря)
	...
	{
		static std::string hint_reference;
		hint_reference = localization->FormatString(LSTR_FMT_S_IS_IN_NO_CODITION_TO_S, pPlayers[pParty->getActiveCharacter()]->pName.c_str(), localization->GetString(LSTR_IDENTIFY_ITEMS));
	...
	else if (pParty->pPickedItem.uItemID >= ITEM_POTION_FLAMING && pParty->pPickedItem.uItemID <= ITEM_POTION_SWIFT || pParty->pPickedItem.uItemID == ITEM_POTION_SLAYING)
	{
