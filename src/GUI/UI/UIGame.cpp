...
void GameUI_ReloadPlayerPortraits(
	int player_id,
	int face_id)
{  // the transition from the zombies to the normal state
	for (uint i = 0; i <= 55; ++i)
	{
		auto filename =
...
{  // PopupWindowForBenefitAndJoinText
	NPCData* pNPC;           // eax@16
...
void GameUI_WritePointedObjectStatusString()
{
	...
	else if (current_screen_type == CURRENT_SCREEN::SCREEN_CHEST)
	{
		...
		{  // if in chest area
			...
			else if (uLastPointedObjectID != 0)
			{  // not found so reset
				game_ui_status_bar_string.clear();
				bForceDrawFooter = 1;
			}
	...
	unsigned int bRedrawOdmMinimap)
{
	...
	if (bWizardEyeActive)
	{
		...
		{  // draw actors(отрисовка монстров и нпс)
		...
		{  // draw items(отрисовка предметов)
