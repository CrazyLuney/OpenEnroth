...
void ShopDialogSellEquip(GUIWindow dialogwin, BuildingType building)
{
	...
	if (HouseUI_CheckIfPlayerCanInteract())
	{
		...
		if (pItemID)
		{
			...
			MERCHANT_PHRASE phrases_id = pPlayers[pParty->getActiveCharacter()]->SelectPhrasesTransaction(item, building, window_SpeakInHouse->wData.val, 3);
			auto str = BuildDialogueString(pMerchantsSellPhrases[phrases_id], pParty->getActiveCharacter() - 1, item, window_SpeakInHouse->wData.val, 3);
			dialogwin.DrawTitleText(pFontArrus, 0, (174 - pFontArrus->CalcTextHeight(str, dialogwin.uFrameWidth, 0)) / 2 + 138, colorTable.White.c16(), str, 3);
		}
	}
}
...
void ShopDialogIdentify(GUIWindow dialogwin, BuildingType building)
{
	...
	if (HouseUI_CheckIfPlayerCanInteract())
	{
		...
		if (pItemID)
		{
			...
			{
				MERCHANT_PHRASE phrases_id = pPlayers[pParty->getActiveCharacter()]->SelectPhrasesTransaction(item, building, window_SpeakInHouse->wData.val, 4);
				str = BuildDialogueString(pMerchantsIdentifyPhrases[phrases_id], pParty->getActiveCharacter() - 1, item, window_SpeakInHouse->wData.val, 4);
			}
			else
			{
				str = BuildDialogueString("%24", pParty->getActiveCharacter() - 1, item, window_SpeakInHouse->wData.val, 4);
			}
...
void ShopDialogRepair(GUIWindow dialogwin, BuildingType building)
{
	...
	if (HouseUI_CheckIfPlayerCanInteract())
	{
		...
		if ((pPlayers[pParty->getActiveCharacter()]->pOwnItems[pItemID - 1].uAttributes & ITEM_BROKEN))
		{
			ItemGen* item = &pPlayers[pParty->getActiveCharacter()]->pInventoryItemList[pItemID - 1];
			MERCHANT_PHRASE phrases_id = pPlayers[pParty->getActiveCharacter()]->SelectPhrasesTransaction(item, building, window_SpeakInHouse->wData.val, 5);
			std::string str = BuildDialogueString(pMerchantsRepairPhrases[phrases_id], pParty->getActiveCharacter() - 1, item, window_SpeakInHouse->wData.val, 5);
			dialogwin.DrawTitleText(pFontArrus, 0, (174 - pFontArrus->CalcTextHeight(str, dialogwin.uFrameWidth, 0)) / 2 + 138, colorTable.White.c16(), str, 3);
		}
	}
}
...
void ShopDialogLearn(GUIWindow dialogwin)
{
	...
	for (int i = pDialogueWindow->pStartingPosActiveItem; i < pDialogueWindow->pNumPresenceButton + pDialogueWindow->pStartingPosActiveItem; ++i)
	{
		...
		if (skillMaxMasteryPerClass[pPlayers[pParty->getActiveCharacter()]->classType][skill] != PLAYER_SKILL_MASTERY_NONE && !pPlayers[pParty->getActiveCharacter()]->pActiveSkills[skill])
		{
...
void WeaponShopWares(GUIWindow dialogwin, bool special)
{
	...
	for (uint i = 0; i < 6; ++i)
	{
		if ((special == 0 && pParty->StandartItemsInShops[window_SpeakInHouse->wData.val][i].uItemID != ITEM_NULL) ||
			(special == 1 && pParty->SpecialItemsInShops[window_SpeakInHouse->wData.val][i].uItemID != ITEM_NULL))
		{
			render->DrawTextureNew(((60 - ((signed int)shop_ui_items_in_store[i]->GetWidth() / 2)) + item_X) / 640.0f, (weapons_Ypos[i] + 30) / 480.0f, shop_ui_items_in_store[i]);
		}
	...
	if (HouseUI_CheckIfPlayerCanInteract())
	{
		...
		if (item_num)
		{
			...
			if (testx >= 0 && testx < 6)
			{
				...
				if (item->uItemID != ITEM_NULL)
				{
					...
					if (pt.x >= testpos && pt.x < (testpos + (signed int)shop_ui_items_in_store[testx]->GetWidth()))
					{
						if (pt.y >= weapons_Ypos[testx] + 30 && pt.y < (weapons_Ypos[testx] + 30 + shop_ui_items_in_store[testx]->GetHeight()))
						{
							...
							if (!StealingMode(pParty->getActiveCharacter()))
							{
								str = BuildDialogueString(
									pMerchantsBuyPhrases[pPlayers[pParty->getActiveCharacter()]->SelectPhrasesTransaction(item, BuildingType_WeaponShop, window_SpeakInHouse->wData.val, 2)],
		...
		else
		{
			...
			dialogwin.DrawShops_next_generation_time_string(pParty->PartyTimes.Shops_next_generation_time[window_SpeakInHouse->wData.val] - pParty->GetPlayingTime());
		}
	}
}
...
void ArmorShopWares(GUIWindow dialogwin, bool special)
{
	...
	for (int i = 0; i < 8; ++i)
	{
			...
			(special == 1 && pParty->SpecialItemsInShops[window_SpeakInHouse->wData.val][i].uItemID != ITEM_NULL))
		{
			...
			{
				render->DrawTextureNew(((86 - (shop_ui_items_in_store[i]->GetWidth() / 2)) + item_x) / 640.0f, (98 - shop_ui_items_in_store[i]->GetHeight()) / 480.0f, shop_ui_items_in_store[i]);
			}
		}
		item_x += 105;
	}
				...
				if (item->uItemID != ITEM_NULL)
				{
					...
					if (pt.x >= testpos && pt.x <= testpos + shop_ui_items_in_store[testx]->GetWidth())
					{
							...
							(pt.y <= 98 && pt.y >= (98 - shop_ui_items_in_store[testx]->GetHeight())))
						{
							...
							{
								str = BuildDialogueString(
									pMerchantsBuyPhrases[pPlayers[pParty->getActiveCharacter()]->SelectPhrasesTransaction(item, BuildingType_ArmorShop, window_SpeakInHouse->wData.val, 2)],
									pParty->getActiveCharacter() - 1,
									item,
									window_SpeakInHouse->wData.val,
									2);
							...
							{
								str = BuildDialogueString(localization->GetString(LSTR_STEAL_ITEM_FMT), pParty->getActiveCharacter() - 1, item, window_SpeakInHouse->wData.val, 2);
							}
							dialogwin.DrawTitleText(pFontArrus, 0, (174 - pFontArrus->CalcTextHeight(str, dialogwin.uFrameWidth, 0)) / 2 + 138, colorTable.White.c16(), str, 3);
						}
					}
				...
				{  // item picking
						...
						pt.x <= testpos + shop_ui_items_in_store[testx]->GetWidth())
					{
								...
								pt.y >= (152 - shop_ui_items_in_store[testx]->GetHeight())))
						{
							...
							{
								str = BuildDialogueString(
									pMerchantsBuyPhrases[pPlayers[pParty->getActiveCharacter()]->SelectPhrasesTransaction(item, building, window_SpeakInHouse->wData.val, 2)],
									pParty->getActiveCharacter() - 1,
									item,
									window_SpeakInHouse->wData.val,
									2);
			...
			if (bought_item->uItemID != ITEM_NULL)
			{
				...
				if (pt.x >= testpos && pt.x <= testpos + shop_ui_items_in_store[testx]->GetWidth())
				{
						...
						(pt.y >= 250 && pt.y <= (250 + shop_ui_items_in_store[testx]->GetHeight())))
					{
						...
						{
							PlayHouseSound(static_cast<HOUSE_ID>(window_SpeakInHouse->wData.val), (HouseSoundID)2);
			...
			if (skillMaxMasteryPerClass[pPlayers[pParty->getActiveCharacter()]->classType][skill] != PLAYER_SKILL_MASTERY_NONE)
			{
				...
				if (!*pSkill)
				{
					if (pParty->GetGold() < uPriceItemService)
					{
						...
						PlayHouseSound(static_cast<HOUSE_ID>(window_SpeakInHouse->wData.val), (HouseSoundID)v55);
						return;
					}
					pParty->TakeGold(uPriceItemService);
					dword_F8B1E4 = 1;
...
void sub_4B1523_showSpellbookInfo(ITEM_TYPE spellItemId)
{
	...
	a1.uFrameWidth -= 12;
	a1.uFrameHeight -= 12;
	v7 = pSpellStats->pInfos[spellId].pName.c_str();
...
void GetHouseGoodbyeSpeech()
{
	...
	if (in_current_building_type != BuildingType_Invalid)
	{
		...
		if (pParty->PartyTimes._shop_ban_times[window_SpeakInHouse->wData.val] <= pParty->GetPlayingTime())
		{
			...
			PlayHouseSound(static_cast<HOUSE_ID>(window_SpeakInHouse->wData.val), (HouseSoundID)(dword_F8B1E4 + 3));
...
{  // not working properly??
	...
	v7 = &pOutdoor->ddm;
	if (uCurrentlyLoadedLevelType != WorldType::Outdoor) v7 = &pIndoor->dlv;
	v7->uReputation += v3;
