...
GUIWindow_JournalBook::GUIWindow_JournalBook() : GUIWindow_Book()
{
	...
	if (books_primary_item_per_page < 29)
	{
			...
			i < books_primary_item_per_page + 29; i++)
		{
			...
			{
				if (!pStorylineText->StoryLine[i + 1].pText.empty())
				{
					auto str = BuildDialogueString(pStorylineText->StoryLine[i + 1].pText, 0, 0, 0, 0, &pParty->PartyTimes.HistoryEventTimes[i]);
...
void GUIWindow_JournalBook::Update()
{
	...
	if (!Journal_limitation_factor[books_primary_item_per_page])
	{
		// for title
		...
		if (!pStorylineText->StoryLine[achieved_awards[books_primary_item_per_page]].pPageTitle.empty())
		{
			journal_window.DrawTitleText(pBook2Font, 0, 22, ui_book_journal_title_color, pStorylineText->StoryLine[achieved_awards[books_primary_item_per_page]].pPageTitle, 3);
		}
	}
	...
	// Press bookmark previous page
	if (BtnUp_flag && books_page_number)
	{
		pAudioPlayer->playUISound(SOUND_openbook);
		--books_page_number;
		books_primary_item_per_page -= (uint8_t)books_num_items_per_page[books_page_number];
	}
	...
	if (achieved_awards[books_primary_item_per_page])
	{
		...
		auto str = BuildDialogueString(pStorylineText->StoryLine[achieved_awards[books_primary_item_per_page]]
