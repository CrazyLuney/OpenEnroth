...
void GUIWindow::HouseDialogManager()
{
	...
	if (v4 || !dword_591080)
	{  // emerald isle ship before quest's done   / на
...
std::string GetDisplayName(Actor* actor)
{
	if (actor->dword_000334_unique_name)
		return pMonsterStats->GetMonsterPlacementInfo(actor->dword_000334_unique_name).pName;
	else
		return pMonsterStats->GetMonsterInfo(actor->pMonsterInfo.uID).pName;
}
