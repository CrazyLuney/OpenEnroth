...
{ // maybe near clipping on projectiles
//  int16_t v1; // fps@1
	uint8_t v2;   // c0@2
	char v3;              // c2@2
	uint8_t v4;   // c3@2
	...
	float x_offset, float y_offset, float z_offset, float scale, int diffuse)
{
	...
	{  // indicies for triangle in sphere
...
{
	return pMobileLightsStack->AddLight(a1->vPosition, a1->uSectorID, uRadius, uDiffuse & 0x000000FF, (uDiffuse & 0x0000FF00) >> 8, (uDiffuse & 0x00FF0000) >> 16, _4E94D3_light_type);
}
...
//----- (004A8BFC) --------------------------------------------------------
int SpellFxRenderer::_4A8BFC_prismatic_light()
{  // for SPELL_LIGHT_PRISMATIC_LIGHT
	uAnimLength =
		8 * pSpriteFrameTable
...
//----- (004775ED) --------------------------------------------------------
int SpellFX_Billboard::SpellFXNearClipAdjust(float NearClip)
{  // near clip adjust - needs diffuse sorting properly??
...
//----- (00477927) --------------------------------------------------------
int SpellFX_Billboard::SpellFXFarClipAdjust(float farclip)
{  // far clip adjust - needs diffuse sorting properly??
...
bool SpellFX_Billboard::SpellFXViewClip()
{
	...
	if (this->uNumVertices <= 0)
	{  //  what?? behaviour needs investigating
