...
namespace data::mm7
{
	void Serialize(const ::Player& src, Player* dst)
	{
		...
		detail::Serialize(src.pName, &dst->pName);
...
namespace data::mm7
{
	void Serialize(const ::Party& src, Party* dst)
	{
		...
		detail::Serialize(src.pHireling1Name, &dst->pHireling1Name);
		detail::Serialize(src.pHireling2Name, &dst->pHireling2Name);
	...
	void Deserialize(const Party& src, ::Party* dst)
	{
		...
		detail::Deserialize(src.pHireling1Name, &dst->pHireling1Name);
		detail::Deserialize(src.pHireling2Name, &dst->pHireling2Name);
...
namespace data::mm6
{
	void Deserialize(const MonsterDesc& src, ::MonsterDesc* dst)
	{
		...
		detail::Deserialize(src.pMonsterName, &dst->pMonsterName);
		for (ActorAnimation i : dst->pSpriteNames.indices())
			detail::Deserialize(src.pSpriteNames[std::to_underlying(i)], &dst->pSpriteNames[i]);
	}
}
...
namespace data::mm7
{
	void Serialize(const ::MonsterDesc& src, MonsterDesc* dst)
	{
		...
		dst->pSoundSampleIDs = src.pSoundSampleIDs;
		detail::Serialize(src.pMonsterName, &dst->pMonsterName);
		for (ActorAnimation i : src.pSpriteNames.indices())
			detail::Serialize(src.pSpriteNames[i], &dst->pSpriteNames[std::to_underlying(i)]);
...
namespace data::mm7
{
	void Serialize(const ::Actor& src, Actor* dst)
	{
		...
		detail::Serialize(src.pActorName, &dst->pActorName);
	...
	{
		detail::Deserialize(src.pActorName, &dst->pActorName);
