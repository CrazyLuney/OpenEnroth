...
void BLV_UpdateDoors()
{
	...
	for (uint i = 0; i < pIndoor->pDoors.size(); ++i)
	{
		...
		for (j = 0; j < door->uNumFaces; ++j)
		{
			...
			face->pFacePlane_old.dist = -static_cast<Vec3i::value_type>(glm::dot(Vec3f(*v17), Vec3f(face->pFacePlane_old.vNormal)));
			face->pFacePlane.dist = -glm::dot(Vec3f(*v17), face->pFacePlane.vNormal);
			...
			if (!decoration->DontDraw())
			{
				if (decoration->uLightRadius)
				{
					...
					pStationaryLightsStack->AddLight(
						Vec3f(pLevelDecorations[i].vPosition) + Vec3f(0, 0, decoration->uDecorationHeight),
...
void IndoorLocation::PrepareDecorationsRenderList_BLV(unsigned int uDecorationID, unsigned int uSectorID)
{
	...
	v8 = pLevelDecorations[uDecorationID]._yawAngle +
		((signed int)TrigLUT.Pi >> 3) - TrigLUT.atan2(pLevelDecorations[uDecorationID].vPosition.x - pCamera3D->vCameraPos.x,
			pLevelDecorations[uDecorationID].vPosition.y - pCamera3D->vCameraPos.y);
	v9 = ((signed int)(TrigLUT.Pi + v8) >> 8) & 7;
...
{  // target from - true on clear
	...
	if (uCurrentlyLoadedLevelType == WorldType::Indoor)
	{
		...
		TrigTableLookup::rotate(32, TrigLUT.HalfPi + AngleToTarget, 0, target, targetmod);
		TrigTableLookup::rotate(32, TrigLUT.HalfPi + AngleToTarget, 0, from, frommod);
		LOS_Occluded2 = Check_LOS_Obscurred_Indoors(targetmod, frommod);

		// offset other side and repeat check
		TrigTableLookup::rotate(32, AngleToTarget - TrigLUT.HalfPi, 0, target, targetmod);
	...
	else if (uCurrentlyLoadedLevelType == WorldType::Outdoor)
	{
		...
		TrigTableLookup::rotate(32, TrigLUT.HalfPi + AngleToTarget, 0, target, targetmod);
		TrigTableLookup::rotate(32, TrigLUT.HalfPi + AngleToTarget, 0, from, frommod);
		LOS_Occluded2 = Check_LOS_Obscurred_Outdoors_Bmodels(targetmod, frommod);

		// offset other side and repeat check
		TrigTableLookup::rotate(32, AngleToTarget - TrigLUT.HalfPi, 0, target, targetmod);
		TrigTableLookup::rotate(32, AngleToTarget - TrigLUT.HalfPi, 0, from, frommod);
		LOS_Occluded1 = Check_LOS_Obscurred_Outdoors_Bmodels(targetmod, frommod);
	}

	bool result{ !LOS_Occluded2 || !LOS_Occluded1 };
...
{  // could this be combined with odm process actions?
	...
	if (fall_start - party_z > 512 && !bFeatherFall && party_z <= floor_z + 1)
	{  // fall damage
	...
	int rotation =
		(static_cast<int64_t>(pEventTimer->dt_fixpoint) * pParty->_yawRotationSpeed * TrigLUT.Pi / 180) >> 16;
	...
	while (pPartyActionQueue->uNumActions)
	{
		...
		case PARTY_TurnLeft:
			if (engine->config->settings.TurnSpeed.value() > 0)
				angle = TrigLUT.TwoPiMask & (angle + (int)engine->config->settings.TurnSpeed.value());
			else
				angle = TrigLUT.TwoPiMask & (angle + static_cast<int>(rotation * fTurnSpeedMultiplier));
			break;
		...
		case PARTY_FastTurnLeft:
			if (engine->config->settings.TurnSpeed.value() > 0)
				...
				angle = TrigLUT.TwoPiMask & (angle + static_cast<int>(2.0f * rotation * fTurnSpeedMultiplier));
			break;
	...
	if (engine->config->settings.WalkSound.value())
	{
		...
		if (pEventTimer->uTimeElapsed)
		{
			...
			int walkDelta = integer_sqrt(static_cast<Vec3i::value_type>(glm::length2(Vec3f(pParty->vPosition - Vec3i(new_party_x, new_party_y, new_party_z)))));
...
int SpawnEncounterMonsters(MapInfo* map_info, int enc_index)
{
	...
	if (uCurrentlyLoadedLevelType == WorldType::Outdoor)
	{
		...
		for (; loop_cnt < 100; ++loop_cnt)
		{
			...
			angle_from_party = (grng->random(TrigLUT.TwoPi) * 2 * pi) / TrigLUT.TwoPi;
	...
	Vec3f* pOutNormal, float* pOutDist)
{
	...
	if (pFace->uNumVertices >= 2)
	{
		...
		{
			FirstPairVec = pVertices[pFace->pVertexIDs[i + 1]] - pVertices[pFace->pVertexIDs[i]];
			SecPairVec = pVertices[pFace->pVertexIDs[i + 2]] - pVertices[pFace->pVertexIDs[i + 1]];

			CrossProd = glm::cross(FirstPairVec, SecPairVec);

			if (CrossProd.x != 0.0 || CrossProd.y != 0.0 || CrossProd.z != 0.0)
			{
				CrossProd = glm::normalize(CrossProd);
				*pOutNormal = CrossProd;
				*pOutDist = -glm::dot(Vec3f(pVertices[pFace->pVertexIDs[i]]), CrossProd);
				return;
			}
		}
	}
