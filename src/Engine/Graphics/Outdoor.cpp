...
double OutdoorLocation::GetFogDensityByTime()
{
	...
	else if (pParty->uCurrentHour >= 5 && pParty->uCurrentHour < 6)
	{  // рассвет
	...
	else if (pParty->uCurrentHour >= 6 && pParty->uCurrentHour < 20)
	{  // день
		pWeather->bNight = false;
		return 0.0;
	}
	else if (pParty->uCurrentHour >= 20 &&
		pParty->uCurrentHour < 21)
	{  // сумерки
		pWeather->bNight = false;
	...
	signed int a5)
{
	...
	if (mapNumberAsInt == MAP_AVLEE && direction == 4)
	{  // to Shoals
	...
	else if (mapNumberAsInt == MAP_SHOALS && direction == 3)
	{  // from Shoals
		uDefaultTravelTime_ByFoot = 1;
...
TileDesc* OutdoorLocation::getTileDescByGrid(int sX, int sY)
{
	...
	if (engine->config->graphics.SeasonsChange.value())
	{
		...
		case 1:            // winter
			if (v3 >= 90)
			{  // Tileset_Grass begins at TileID = 90
...
void ODM_ProcessPartyActions()
{
	...
	if (pParty->vPosition.z <= currentGroundLevel)
	{  // landing from flight
		ceilingHeight = -1;
	...
	if (partyNotTouchingFloor && !pParty->bFlying)
	{  // add gravity
