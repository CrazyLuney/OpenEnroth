...
struct SpriteFrameTable
{
	~SpriteFrameTable()
	{
		uNumSpriteFrames = 0;
		uNumEFrames = 0;
		pSpriteSFrames.reset();
		pSpritePFrames.reset();
		pSpriteEFrames.reset();
	}

	void ToFile();
	void FromFile(const Blob& data_mm6, const Blob& data_mm7, const Blob& data_mm8);
	void ResetLoadedFlags();
	void InitializeSprite(size_t uSpriteID);
	...
	std::size_t uNumSpriteFrames = 0;
	std::size_t uNumEFrames = 0;

	std::unique_ptr<SpriteFrame[]> pSpriteSFrames;
	std::unique_ptr<SpriteFrame*[]> pSpritePFrames;
	std::unique_ptr<uint16_t[]> pSpriteEFrames;
};
