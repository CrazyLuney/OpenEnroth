...
SPELL_TYPE ParseSpellType(struct FrameTableTxtLine* tbl, int* next_token)
{
	...
	if (iequals(tbl->pProperties[0], "Dispel"))
	{  // dispel magic
		++* next_token;
		return SPELL_LIGHT_DISPEL_MAGIC;
	}
	else if (iequals(tbl->pProperties[0], "Day"))
	{  // day of protection
		*next_token += 2;
		return SPELL_LIGHT_DAY_OF_PROTECTION;
	}
	else if (iequals(tbl->pProperties[0], "Hour"))
	{  // hour  of power
		*next_token += 2;
	...
	else if (iequals(tbl->pProperties[0], "Power"))
	{  // power cure
		++* next_token;
		return SPELL_BODY_POWER_CURE;
	}
	else if (iequals(tbl->pProperties[0], "Meteor"))
	{  // meteot shower
		++* next_token;
		return SPELL_FIRE_METEOR_SHOWER;
	}
	else if (iequals(tbl->pProperties[0], "Lightning"))
	{  // Lightning bolt
		++* next_token;
	...
	else if (iequals(tbl->pProperties[0], "Pain"))
	{  // pain reflection
		++* next_token;
	...
	else if (iequals(tbl->pProperties[0], "Toxic"))
	{  // toxic cloud
		++* next_token;
