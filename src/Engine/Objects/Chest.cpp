...
bool Chest::Open(int uChestID)
{
	...
	if (chest->Trapped() && pMapID)
	{
			...
			2 * pMapStats->pInfos[pMapID].LockX5)
		{
			...
			TrigTableLookup::rotate(pDepth, yawAngle, pitchAngle, v, pOut);
...
int Chest::PutItemInChest(int position, ItemGen* put_item, int uChestID)
{
	...
	{  // no position specified
		...
		if (test_pos == max_size)
		{  // limits check no room
...
void Chest::PlaceItemAt(unsigned int put_cell_pos, unsigned int item_at_cell, int uChestID)
{  // only used for setup?
...
void Chest::GrabItem(bool all)
{  // new fucntion to grab items from chest using spacebar
