...
void ObjectList::FromFile(const Blob& data_mm6, const Blob& data_mm7, const Blob& data_mm8)
{
	...
	constexpr auto s = sizeof(ObjectDesc);
	constexpr auto t = sizeof(ObjectDesc_mm6);
	...
	memcpy(pObjects.data(), reinterpret_cast<const char*>(data_mm7.data()) + 4, num_mm7_objs * sizeof(ObjectDesc));
