...
void CastSpellInfoHelpers::castSpell()
{
	...
	{  // cycle through spell queue
		...
		if (!pCastSpell->forced_spell_skill_level)
		{
			...
			{  // free spells at midnight or midday
				uRequiredMana = 0;
			}
		}
		...
		if (!pCastSpell->forced_spell_skill_level // temple or NPC spells must not fail because of curse
			...
			&& grng->random(100) < 50)
		{  // player is cursed and have a chance to fail spell casting
		...
		else
		{
			...
			case SPELL_AIR_IMPLOSION:
			{
				...
				initSpellSprite(&pSpellSprite, spell_level, spell_mastery, pCastSpell);
				pSpellSprite.vPosition = Vec3i(pActors[monster_id].vPosition) + Vec3i(0, 0, pActors[monster_id].uActorHeight / 2);
			...
			case SPELL_SPIRIT_SPIRIT_LASH:
			{
				...
				int monster_id = PID_ID(spell_targeted_at);
				float dist = glm::length(Vec3f(pActors[monster_id].vPosition) - Vec3f(pParty->vPosition));
				if (dist <= 307.2)
				{
					Vec3i spell_velocity = Vec3i(0, 0, 0);
					initSpellSprite(&pSpellSprite, spell_level, spell_mastery, pCastSpell);
					pSpellSprite.vPosition = Vec3i(pActors[monster_id].vPosition) - Vec3i(0, 0, pActors[monster_id].uActorHeight * -0.8);
			...
			case SPELL_FIRE_INFERNO:
			{
				...
				{
					Vec3i spell_velocity = Vec3i(0, 0, 0);
					pSpellSprite.vPosition = Vec3i(actor->vPosition) - Vec3i(0, 0, actor->uActorHeight * -0.8);
			...
			case SPELL_WATER_ENCHANT_ITEM:
			{
					...
					!spell_item_to_enchant->IsBroken())
				{
					...
					if ((spell_item_to_enchant->GetValue() < 450 && !IsWeapon(this_equip_type)) ||  // not weapons
						(spell_item_to_enchant->GetValue() < 250 && IsWeapon(this_equip_type)))
					{  // weapons
					...
					else
					{
						...
						else
						{
							...
							if (rnd < 80 && IsPassiveEquipment(this_equip_type))
							{ // chance to roll standard enchantment on non-weapons
								int ench_found = 0;
								...
								{
									const auto& this_bonus_stat = pItemTable->pEnchantments[norm_ench_loop].pBonusStat;
									if (!this_bonus_stat.empty())
									{
							...
							{ // weapons or we won the lottery for special enchantment
								int ench_found = 0;
								int to_item_apply_sum = 0;
								int ench_array[100] = { 0 };
								...
								if (pItemTable->pSpecialEnchantments_count > 0)
								{
									...
									{
										const auto& this_bonus_statement = pItemTable->pSpecialEnchantments[spec_ench_loop].pBonusStatement;
										if (!this_bonus_statement.empty())
										{
											...
											int this_to_apply = pItemTable->pSpecialEnchantments[spec_ench_loop].to_item[this_equip_type];
											to_item_apply_sum += this_to_apply;
											if (this_to_apply)
											{
								...
								for (step = 0; step < ench_found; step++)
								{
									current_item_apply_sum += pItemTable->pSpecialEnchantments[(ITEM_ENCHANTMENT)ench_array[step]].to_item[this_equip_type];
									if (current_item_apply_sum >= target_item_apply_rand)
									{
										break;
									}
			...
			case SPELL_EARTH_DEATH_BLOSSOM:
			{
				...
				int spell_speed = pObjectList->pObjects[pSpellSprite.uObjectDescID].uSpeed;
				if (pSpellSprite.Create(pParty->_viewYaw, TrigLUT.HalfPi / 2, spell_speed, 0) != -1 &&
					pParty->bTurnBasedModeOn)
				{
					++pTurnEngine->pending_actions;
				}
