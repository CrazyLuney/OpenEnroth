...
AnyConfigEntry::AnyConfigEntry(ConfigSection* section, const std::string& name, const std::string& description, AnyHandler* handler, std::any defaultValue, Validator validator)
{
	assert(section);
	assert(handler);
	assert(!name.empty());
