...
int Game::run()
{
	...
	keyboardInputHandler = std::make_shared<KeyboardInputHandler>(_windowHandler->KeyboardController(), keyboardActionMapping);
	...
	if (!pCurrentFrameMessageQueue->Empty())
	{
		while (true)
		{
			...
			case UIMSG_DebugLearnSkills:
				for (Player& player : pParty->pPlayers)
				{ // loop over players
					for (PLAYER_SKILL_TYPE ski : AllSkills())
					{  // loop over skills
